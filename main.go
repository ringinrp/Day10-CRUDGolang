...
func main() {
	...
	route.HandleFunc("/project/addproject", addproject).Methods("POST")
...
func home(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		rows, _ := connection.Conn.Query(context.Background(), `SELECT id, project_name, start_date, end_date,description, image, technologies FROM tb_project`)
		for rows.Next() {

			err := rows.Scan(&item.ID, &item.ProjectName, &item.StartDate, &item.EndDate, &item.Description, &item.Image, &item.Technologies)
...
func editProject(w http.ResponseWriter, r *http.Request) {
	...
	// 	// DATABASE CONNECTION
	// 	_, err = connection.Conn.Exec(context.Background(), `UPDATE public.tb_project
	// 	SET "project_name"=$1, "start_date"=$2, "end_date"=$3, "description"=$4, "technologies"=$5,
	// 	WHERE "ID"=$7`, ProjectName, StartDate, EndDate, Description, Technologies, ID)
	// 	// ERROR HANDLING
	// 	if err != nil {
	// 		w.WriteHeader(http.StatusInternalServerError)
	// 		w.Write([]byte("message : " + err.Error()))
